...
ws.onopen = function() {
    ws.send(JSON.stringify(grid_Info));
    console.log("connected sending grid_Info")
}
...
function setup() {
    ...
    for (let i = 0; i < grid_Info.x * grid_Info.y; i++) {
        ...
        grid_Div.addEventListener("click", function() {
            if (grid_Div.classList.contains("off")) {
                grid_On(grid_Div);
                send_OSC("/grid/led/key", [grid_Div.x, grid_Div.y, 1]);
            } else if (grid_Div.classList.contains("on")) {
                grid_Off(grid_Div);
                send_OSC("/grid/led/key", [grid_Div.x, grid_Div.y, 0]);
            }
        });
    }
...
function message_from_Websocket() {
...
function send_Info(oscMessage) {
    ...
    const sysInfo = {
        ...
        args: [
            ...
            ["/sys/size", [grid_Info.y, grid_Info.x]]
        ]
    };
...
function osc_Map(oscMessage) {
    ...
    if (oscMessage.args[1] > grid_Info.y - 8) {
        oscMessage.args[1] = grid_Info.y - 8;
    } else if (oscMessage.args[1] < 0) {
        oscMessage.args[1] = 0;
    }

    for (let i = 0; i < grid_Info.y; i++) {
...
function osc_RC (oscMessage, rc) {
    ...
    if (offsetX < 0) {
        offsetX = 0;
    } else if (rc == "row" && offsetX > grid_Info.y) {
        offsetX = grid_Info.y;
    } else if (rc == "col" && offsetX > grid_Info.x - 1) {
        offsetX = grid_Info.x - 1;
    }

    //constrains y from 0-7 if in rows or to 0 if in cols
    if (offsetY < 0) {
        offsetY = 0;
    } else if (rc == "row" && offsetY > grid_Info.y - 1) {
        offsetY = grid_Info.y - 1;
    ...
    if (oscMessage.args.length > 2) {
        ...
        if (rc == "row") {
            selectedDivs = Array.from(gridDivs).filter(box => box.x >= 0 + offsetX && box.x < grid_Info.y + offsetX && box.y === offsetY);
        } else if (rc == "col") {
            selectedDivs = Array.from(gridDivs).filter(box => box.x === offsetX && box.y >= 0 + offsetY && box.y < grid_Info.x + offsetY);
        }
